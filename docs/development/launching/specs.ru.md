...
1. **Boot (Загрузка системы)**:
    ...
    - **Загрузка метаданных**: Считывание статической информации о приложении (Название игры, Начальный заголовок окна, Версия) из основного конфига до полной инициализации движка.
    ...
    - **Локализация**: Загрузка ресурсов Project Fluent (`.ftl`) и шрифтов до входа в меню.
2. **Splash (Заставки)**:
...
Проект использует систему **Project Fluent (.ftl)** для управления текстовыми ресурсами:

- **Грамматика и склонения**: Нативная поддержка сложных правил (напр., русские формы множественного числа).
- **Доступ по ключам**: Код обращается к уникальным идентификаторам (напр., `menu-start-button`), а не к тексту.
- **Динамический контент**: Поддержка переменных (напр., `welcome-user = Добро пожаловать, { $user }!`).
- **Локализованное аудио**: Поддержка региональных аудиофайлов (озвучка, специфические звуки), хранящихся внутри структуры локали.
- **Изоляция и Fallback**: Каждый язык хранится в своей поддиректории внутри `assets/locales/`. При отсутствии локализованного ассета система автоматически откатывается к основному языку (fallback).

### Стандартные аудио-эффекты UI

| Файл | Назначение |
| ---- | ---------- |
| `click.ogg` | Основное действие (нажатие кнопки). |
| `select.ogg` | Наведение на кнопку (hover) или выделение пункта меню. |
| `back.ogg` | Выход из подменю (напр. из Настроек назад в Главное меню). |
| `confirmation.ogg` | Успешное действие (напр. сохранение настроек или завершение загрузки). |
| `error.ogg` | Звуковая обратная связь при ошибках (если не удалось загрузить конфиг). |
| `scroll.ogg` | Для прокрутки списков (в меню настроек или при выборе слотов сохранения). |
| `toggle.ogg` | Для переключателей в настройках (Вкл/Выкл VSync и т.д.). |
| `open.ogg` / `close.ogg` | Опционально: анимация появления/скрытия панелей меню. |
| `maximize.ogg` / `minimize.ogg` | Опционально: масштабирование интерфейса или полноэкранные переходы. |
...
│   ├── audio/              # Общие звуки и музыка
│   ├── locales/            # Локализация (Модульная AAA структура)
│   │   ├── en-US/          # Английский
│   │   │   ├── text/       # .ftl файлы (menu, items, dialogs)
│   │   │   └── audio/      # Английская озвучка и региональные звуки
│   │   ├── ru-RU/          # Русский
│   │   │   ├── text/       # .ftl файлы (menu, items, dialogs)
│   │   │   └── audio/      # Русская озвучка и региональные звуки
│   │   └── index.toml      # Манифест языков (метаданные всех локалей)
