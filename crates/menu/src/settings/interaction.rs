...
use super::{AudioSettings, DisplaySettings, UserSettings};
use crate::modal::{ModalState, ModalType};
use bevy::prelude::*;
use launcher::config::AppPaths;
use launcher::config::settings::save_settings;
use theme::{Theme, ThemeColors, constants};
...
#[derive(Message, Debug, Clone)] // Changed from Message to Event for Bevy 0.15 compat/standard
...
pub fn auto_save_settings(
    ...
    settings: Res<UserSettings>,
    paths: Res<AppPaths>,
    _error_events: MessageWriter<SettingsSaveError>,
) {
    ...
    if timer.0.is_finished() {
        ...
        if let Err(e) = save_settings(&paths, &settings) {
            ...
            commands.insert_resource(ModalState {
                active: Some(ModalType::Error(error_msg)),
            });
        }
        timer.0.pause();
    }
