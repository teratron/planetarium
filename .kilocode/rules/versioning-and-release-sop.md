...
This document establishes a strict, repeatable standard operating procedure (SOP) for project version control, ensuring consistency across all project artifacts and clear communication of changes. This rule must be followed for any changes intended for a public release.

## Trigger

This procedure is performed as a final step after all development, feature integration, bug fixing, and quality control work is completed, as well as after the main branch is ready to release a new version.
...
2. **Update Version String**: Systematically update the project's version number to the newly determined version in all designated files. The update must be consistent across:
    ...
    * Project version constants/variables if they exist
...
4. **Create Release Commit**: Stage all modified files and create a single, dedicated commit for the version bump. The commit message should follow a conventional format.
    * **Command:** `git add README.md CHANGELOG.md docs/`
